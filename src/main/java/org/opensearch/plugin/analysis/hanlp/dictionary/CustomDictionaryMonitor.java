...
import org.opensearch.plugin.analysis.hanlp.utils.CustomDictionaryUtils;
...
/**
 * The custom dictionary monitor.
 *
 * @author Rory Ye
 */
public class CustomDictionaryMonitor implements Runnable {

    private static final Logger logger = LogManager.getLogger(CustomDictionaryMonitor.class);

    CustomDictionaryMonitor() {
    ...
    public void run() {
        ...
        if (isModified) {
            ...
            try {
                AccessController.doPrivileged((PrivilegedAction) CustomDictionaryUtils::reload);
            } catch (Exception e) {
